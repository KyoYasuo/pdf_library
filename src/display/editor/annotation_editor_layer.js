...
class AnnotationEditorLayer {
  ...
  static #editorTypes = new Map(
    [FreeTextEditor, InkEditor, SquareEditor, CircleEditor, StampEditor, HighlightEditor].map(type => [
      type._editorType,
      type,
    ])
  );
  ...
  updateMode(mode = this.#uiManager.getMode()) {
    ...
    switch (mode) {
      ...
      case AnnotationEditorType.CIRCLE:
        // We always want to have an Circle editor ready to draw in.
        this.addCircleEditorIfNeeded(false);
  ...
  /**
   * Adds a square editor if needed.
   * 
   * @param {boolean} isCommitting - Indicates if we are in the process of committing changes.
   */
  addSquareEditorIfNeeded(isCommitting) {
    // Check if we are in square mode
    if (this.#uiManager.getMode() !== AnnotationEditorType.SQUARE) {
      // We don't want to add a square editor if we're not in square mode!
      return;
    }
    // Check if we are removing an editor
    if (!isCommitting) {
      // We're removing an editor but an empty one can already exist so in this
      // case we don't need to create a new one.
      for (const editor of this.#editors.values()) {
    ...
    // Create a new square editor and set it to the background
    const editor = this.#createAndAddNewEditor(
      { offsetX: 0, offsetY: 0 },
      /* isCentered = */ false,
  ...
  /**
 * Adds a circle editor if needed.
 * 
 * @param {boolean} isCommitting - Indicates if we are in the process of committing changes.
 */
  addCircleEditorIfNeeded(isCommitting) {
    // Check if we are in circle mode
    if (this.#uiManager.getMode() !== AnnotationEditorType.CIRCLE) {
      // We don't want to add a circle editor if we're not in circle mode!
      return;
    }
    // Check if we are removing an editor
    if (!isCommitting) {
    ...
    // Create a new circle editor and set it to the background
    const editor = this.#createAndAddNewEditor(
      { offsetX: 0, offsetY: 0 },
      /* isCentered = */ false,
